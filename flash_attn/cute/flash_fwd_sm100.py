...
class FlashAttentionForwardSm100:
            ...
            if has_work:
                ...
                for stage in cutlass.range_constexpr(self.q_stage):
                    ...
                    self.correction_epilogue(
                        ...
                        v_descale=v_descale if const_expr(self.p_in_smem) else None,
                    )
    ...
    def correction_epilogue(
        ...
        v_descale_eff = v_descale if v_descale is not None else Float32(1.0)
        for i in cutlass.range(self.head_dim_v_padded // corr_tile_size, unroll_full=True):
            ...
            if const_expr(zero_output):
                ...
                if const_expr(use_per_elem_scale):
                    # The 16-datapath load gives each thread values from multiple rows.
                    for k in cutlass.range(cute.size(tOrO_frg), unroll_full=True):
                        row_idx = tOcO_t2r_i[k][0]
                        row_sum = sScale[row_idx + stage * self.m_block_size]
                        scale_k = cute.arch.rcp_approx(row_sum) * v_descale_eff
                        tOrO_frg[k] = tOrO_frg[k] * scale_k
                else:
                    for j in cutlass.range(0, cute.size(tOrO_frg), 2, unroll_full=True):
                        tOrO_frg[j], tOrO_frg[j + 1] = cute.arch.mul_packed_f32x2(
                            (tOrO_frg[j], tOrO_frg[j + 1]), (scale, scale)
